...
%Converted the resource manager to be generic and to use smart pointers
	...
	The initial versions of the resource manager used raw pointers, this caused a few issues with leaving dangling pointers when trying to right modern code, to resolve this issue smart pointers were used. The shared pointer class from the C++ standard library was the perfect type of smart pointer for use in this case as the resources were being shared an additional time with each use. Shared pointers also allowed for getting the use count of itself as opposed to raw pointers which have no idea how many times they are being used.
